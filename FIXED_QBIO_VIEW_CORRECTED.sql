-- ====================================================================
-- ИСПРАВЛЕННЫЙ SQL VIEW ДЛЯ QBIO - ПРАВИЛЬНАЯ СТРУКТУРА ТАБЛИЦ
-- ====================================================================
-- ПРОБЛЕМА: Перепутаны колонки dao_pool_snapshots и lp_position_snapshots
-- РЕШЕНИЕ: Используем правильные колонки для каждой таблицы
-- ====================================================================
...
bio_price AS (
  -- 🔧 ИСПРАВЛЕНО: Получаем цену BIO из lp_position_snapshots
  WITH bio_token0_prices AS (
    SELECT token0_price_usd as price, created_at
    FROM lp_position_snapshots 
    WHERE token0_symbol = 'BIO' AND token0_price_usd > 0
    AND created_at >= '2025-07-30T00:00:00Z'
  ),
  bio_token1_prices AS (
    SELECT token1_price_usd as price, created_at
    FROM lp_position_snapshots 
    WHERE token1_symbol = 'BIO' AND token1_price_usd > 0
    AND created_at >= '2025-07-30T00:00:00Z'
  ),
  all_bio_prices AS (
    SELECT price, created_at FROM bio_token0_prices
    UNION ALL
    SELECT price, created_at FROM bio_token1_prices
  )
  SELECT price as bio_price_usd
  FROM all_bio_prices 
  ORDER BY created_at DESC 
  LIMIT 1
),
latest_bio_pairs AS (
  -- 🔧 ИСПРАВЛЕНО: Последние данные о BIO парах из dao_pool_snapshots
  SELECT 
    token_symbol,
    network,
    pool_name,
  ...
  WHERE 
    ...
    AND is_bio_pair = true  -- 🔧 ИСПОЛЬЗУЕМ is_bio_pair вместо логики token0/token1
    AND token_symbol <> 'BIO'  -- Исключаем сам BIO
...
aggregated_positions AS (
  -- 🔧 ИСПРАВЛЕНО: Агрегируем позиции по токену из lp_position_snapshots
  SELECT 
    -- Определяем токен из BIO пары (используем правильные колонки)
    CASE 
...
unified_data AS (
  ...
  -- 🔧 ТЕПЕРЬ ВСЕ ТОКЕНЫ БУДУТ ВКЛЮЧЕНЫ (данные уже отфильтрованы в CTE)
  WHERE tm.token_symbol IS NOT NULL  -- Только токены с метриками
)
SELECT 
