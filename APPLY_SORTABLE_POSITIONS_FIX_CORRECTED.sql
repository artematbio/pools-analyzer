-- =====================================================
-- ИСПРАВЛЕННАЯ ВЕРСИЯ: КОЛОНКА "OUR POSITIONS" ДЛЯ СОРТИРОВКИ
-- =====================================================
-- 
-- ИСПРАВЛЕНИЯ:
-- ✅ dao_pools_snapshot → dao_pool_snapshots (правильное название таблицы)
-- ✅ snapshot_time → snapshot_timestamp (правильное название поля)
-- ✅ Упрощенная логика JOIN'ов для совместимости
-- =====================================================

-- Создаем улучшенное представление с правильными типами данных
CREATE OR REPLACE VIEW bio_dao_lp_support_v2 AS
WITH latest_dao_snapshots AS (
    -- Берем последние снапшоты DAO пулов (правильное название таблицы)
    SELECT DISTINCT ON (pool_address, network) 
        pool_address,
        pool_name,
        network,
        dex,
        ...
        is_bio_pair,
        snapshot_timestamp,  -- Правильное название поля
        created_at
    FROM dao_pool_snapshots  -- Правильное название таблицы
    ORDER BY pool_address, network, snapshot_timestamp DESC
),
our_positions_summary AS (
    ...
    GROUP BY pool_name, network, pool_address
)
SELECT 
    -- Основная информация
    d.pool_address,
    ...
    -- Форматированная строка для отображения (опционально)
    CASE 
    ...
    -- Временные метки
    d.snapshot_timestamp,  -- Правильное название поля
    d.created_at,
...
COMMENT ON VIEW bio_dao_lp_support_v2 IS 'Исправленное представление BIO DAO LP поддержки с правильными названиями таблиц и полей. Колонка our_positions_usd теперь NUMERIC для сортировки.';

-- =====================================================
-- ПРИМЕРЫ ИСПОЛЬЗОВАНИЯ ПОСЛЕ ПРИМЕНЕНИЯ
-- =====================================================
